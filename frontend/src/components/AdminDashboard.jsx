...
import { Layout, Users, FileText, Calendar, ArrowLeft, Loader2, Download, ExternalLink } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <div className="mb-8">
            <h1 className="text-3xl font-extrabold text-slate-900 mb-2">Visão Geral de Leads</h1>
            <p className="text-slate-600">Acompanhe as requisições de análise profunda em tempo real.</p>
        </div>

        {loading ? (
            <div className="flex flex-col items-center justify-center p-20">
                <Loader2 className="animate-spin text-blue-600 mb-4" size={48} />
                <p className="text-slate-500 font-medium tracking-wide italic">Carregando dados dos leads...</p>
            </div>
        ) : leads.length === 0 ? (
                            ...
                            {leads.map((lead) => (
                                <tr key={lead.id} className="hover:bg-slate-50 transition-colors bg-white">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        <div className="bg-slate-100 text-[10px] inline-block px-1.5 py-0.5 rounded font-bold text-slate-500 uppercase mt-1">CV: {lead.filename}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button className="p-2 text-slate-400 hover:text-blue-600 transition-colors title='Ver Detalhes'">
